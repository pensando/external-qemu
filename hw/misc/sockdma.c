...
static void data_avail(SockDMAState *s)
{
    ...
    if (!remote_exists(s)) {
        s->regs.status &= ~STATUS_DATA_AVAIL_MASK;
        return;
    }
    ...
    if (ret < 0) {
        if (ret == QIO_CHANNEL_ERR_BLOCK) {
            ...
            s->regs.status &= (~STATUS_DATA_AVAIL_MASK & ~STATUS_SUCCESS_MASK);
...
static uint64_t sockdma_read_csr(void *opaque, hwaddr addr, unsigned int size)
{
    ...
    case SOCKDMA_REGS_CTRL_OFF:
        ...
        val = s->regs.ctrl;
        break;
    case SOCKDMA_REGS_STATUS_OFF:
        assert(size == sizeof(uint32_t));
        if ((s->regs.ctrl & CTRL_INT_MASK) == 0 && (s->regs.status & STATUS_SUCCESS_MASK)) {
         ...
         * bad happens */
        s->regs.status |= STATUS_SUCCESS_MASK;
        break;
...
static void send_data(SockDMAState *s)
{
    ...
    ret = qio_channel_write(ioc, s->storage, s->storage_size, &err);
    if (ret <= 0) {
